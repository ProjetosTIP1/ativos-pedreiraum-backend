...
class AssetApprovalService:
    ...
    async def approve_asset(self, asset_id: UUID, final_data: Optional[dict] = None) -> Optional[Asset]:
        """
        ...
        if asset.status != AssetStatus.PENDING:
            raise ValueError(f"Asset is not in PENDING status. Current status: {asset.status}")
    ...
    async def reject_asset(self, asset_id: UUID, reason: Optional[str] = None) -> Optional[Asset]:
        """
